...
CREATE TABLE `punto_control` (
  ...
  `limite_paquete` int DEFAULT NULL,
  `moneda` varchar(5) DEFAULT NULL,
  `tarifa_local` int DEFAULT NULL,
  `id_tarifa_global` int DEFAULT NULL,
  PRIMARY KEY (`id_punto_control`),
  KEY `id_ruta` (`id_ruta`),
  KEY `id_tarifa_global` (`id_tarifa_global`),
  CONSTRAINT `punto_control_ibfk_1` FOREIGN KEY (`id_ruta`) REFERENCES `ruta` (`id_ruta`),
  CONSTRAINT `punto_control_ibfk_2` FOREIGN KEY (`id_tarifa_global`) REFERENCES `tarifa` (`id_tarifa`)
